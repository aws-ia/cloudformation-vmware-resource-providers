...
def create_sddc_json_v2(authentication: VMCAuth, strProdURL: str, org_id: str, name: str, region: str, host_count: int, host_type: str, cidr_block: str, skip_creating_vxlan: bool = False, vxlan_subnet: str = None, provider_type="AWS", aws_account_id = None, link_subnet_id = None, sddc_size="medium"):
    ...
    if resp.status_code == 201 or resp.status_code == 202 or resp.status_code == 202:
        ...
        print(f'Error {resp.status_code} returned by call to {my_url}, payload: {json.dumps(call_data,indent=4)}')
...
def get_sddc_task_progress_json(strProdURL, authentication: VMCAuth, orgid, sddcid, task_id):
    myHeader = {'csp-auth-token': authentication.access_token}
...
def get_sddc_task_details_json(strProdURL, authentication: VMCAuth, orgid, sddcid, task_id, retrieve_progress=False) -> dict:
    task_info = {}
    json_response_task = get_sddc_tasks_json(strProdURL, authentication, orgid, sddcid, task_id)
    if json_response_task:
        ...
        if retrieve_progress:
            if "provider_assigned_id" in json_response_task.keys():
                ...
                json_response_progress = get_sddc_task_progress_json(strProdURL, authentication, orgid, sddcid, json_response_task["provider_assigned_id"])
                if json_response_progress:
                    task_info["progress_percent"] = json_response_progress["progress_percent"]
                    task_info["estimated_remaining_minutes"] = json_response_progress["estimated_remaining_minutes"]                
...
def check_task_status_v2(strProdURL, authentication: VMCAuth, orgId, sddcId, taskId):
    task_details = get_sddc_task_details_json(strProdURL, authentication, orgId, sddcId, taskId, retrieve_progress=True)
...
def wait_for_task_v2(strProdURL, authentication: VMCAuth, orgId, sddcId, task_id):
    ...
    task_details = get_sddc_task_details_json(strProdURL, authentication, orgId, sddcId, task_id, retrieve_progress=True)
    ...
    while task_not_ready and details_ctr < MAX_RETRIEVAL_ATTEMPTS:
        ...
        details_ctr += 1
        print(f'Attempting to retrieve task ({details_ctr}/{MAX_RETRIEVAL_ATTEMPTS})...')
        time.sleep(10)
        authentication.check_access_token_expiration()
        task_details = get_sddc_task_details_json(strProdURL, authentication, orgId, sddcId, task_id)
        print(task_details)
